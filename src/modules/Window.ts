...
export class Window implements WindowStructure, WindowOptions {
    ...
    public readonly SetSize = <W extends number, H extends number>(options: { width: W; height: H }): [number, number] => {
    ...
    public readonly IsInBounds = (options: { x: number; y: number; width: number; height: number; sizeIncluded?: boolean }): boolean => {
        const { x, y, width, height } = options;
